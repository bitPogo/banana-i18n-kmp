...
letter          := [A-Za-z]
escaped         := '\' [\u0021-\u002F \u003A-\u0040 \u005B-\u0060 \u007B-\u007E]
nonascii        := [{::TITLECASE_LETTER::}{::MODIFIER_LETTER::}{::OTHER_LETTER::}]+
literal         := .
```
...
=== Number
is a family of plugins, which has a common base as well as their specific implementation.
...
<selectionOptions>      := '|' <space> <selector> <space> <repeatArgument> <nestedRuleOrText> <space> <selectionOptions>
                        | '|' <space> <selector> <space> <repeatArgument> <nestedRuleOrText>
